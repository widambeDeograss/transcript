...
def add_score_for(request, id):
    ...
    if request.method == 'POST':
        ...
        for s in range(0,len(ids)):      # iterate over the list of student ids gathered above
            ...
            obj.comment = obj.get_comment(obj.ca, obj.grade,obj.exam )
...
@login_required
@lecturer_required
def completed_semesters(request):
    completed_sems = Result.objects.all()
    return render(request, 'course/course_allocation_view.html', {"completed_sems": completed_sems})
...
def result_sheet_pdf_view(request, id):
    ...
    no_of_fail = TakenCourse.objects.filter(course__pk=id, comment="CARRY").count()
    no_of_supp = TakenCourse.objects.filter(course__pk=id, comment="SUPP").count()
    ...
    normal.fontSize = 12
    normal.leading = 15
    title = "<b> "+str(current_semester) + " Semester " + str(current_session) + "</b>"
    ...
    normal.fontSize = 12
    normal.leading = 15
    title = "<b> " + str(course.courseTitle) + " Result Sheet</b>"
    title = Paragraph(title.upper(), normal)
    Story.append(title)
    Story.append(Spacer(1, 0.1 * inch))
    ...
    title = "<b> NTA Level: </b>" + str(level.course.level)
    title = Paragraph(title.upper(), normal)
    Story.append(title)
    ...
    [Paragraph("<b></b> _____________________________", styles["Normal"]), Paragraph("<b>No. of SUPPLIMENTARY: </b>" + str(no_of_supp), style_right)]]
...
def generate_pdf(request, *args, **kwargs):
    template = loader.get_template('result/students_results.html')
    current_semester = Semester.objects.get(is_current_semester=True)
    current_session = Session.objects.get(is_current_session=True)
    ...
    for i in sl:
        ...
        result = [entry for entry in Result.objects.filter(student=student).values('gpa')]
        print(result)
    ...
    context = {"student_list": mystudentlist, "year_of_study": current_session, "semester": current_semester}
    # print(context)

    return render(request, 'result/students_results.html', context)
